...
const EditStudent=async function(req,res){
    ...
    if(!(name || subject || marks)) return res.status(400).send({status:false,message:'please enter at least one of them [name,subject marks]'})


       const student=await studentSchema.findOne({user:userName,name:name,subject,isDeleted:false})

       
       if(!student){
        return res.status(404).send({status:fasle,massage:"student did not found"})
       }


       
       data.user=req.userName
       const newStu=await studentSchema.fo(data)
...
module.exports={createUser,logInUser,addStudent}